...
begin
  result:={$IFNDEF CLR}PChar{$ENDIF}(glGetString(GL_RENDERER));
end;
...
function TotalMemoryAllocated:Cardinal;
begin
  result:={$IFDEF D16}
            MemoryUsed;
          {$ELSE}
            {$IFDEF FASTMM}FastGetHeapStatus{$ELSE}GetHeapStatus{$ENDIF}.TotalAllocated;
          {$ENDIF}
end;
...
procedure TMakerEditor1.PageControl2Change(Sender: TObject);
  ...
  begin
     ...
     begin
       ...
       AddExtensions({$IFNDEF CLR}PChar{$ENDIF}(glGetString(GL_EXTENSIONS)));

       wglGetExtensionsStringARB := wglGetProcAddress('wglGetExtensionsStringARB');
       if Assigned(wglGetExtensionsStringARB) then
       begin
         tmp:={$IFNDEF CLR}PChar{$ENDIF}(wglGetExtensionsStringARB(wglGetCurrentDC));
       ...
       AddExtensions({$IFNDEF CLR}PChar{$ENDIF}(gluGetString(GLU_EXTENSIONS)));

       MemoExtensions.Lines.EndUpdate;

       MemoGLInfo.Lines.Add('');

       MemoGLInfo.Lines.Add('OpenGL Vendor: '+{$IFNDEF CLR}PChar{$ENDIF}(glGetString(GL_VENDOR)));
       MemoGLInfo.Lines.Add('OpenGL Version: '+{$IFNDEF CLR}PChar{$ENDIF}(glGetString(GL_VERSION)));
       MemoGLInfo.Lines.Add('OpenGL Renderer: '+OpenGLRenderer);
       MemoGLInfo.Lines.Add('GLU Version: '+{$IFNDEF CLR}PChar{$ENDIF}(gluGetString(GLU_VERSION)));
...
begin
  ...
  if PageControl2.ActivePage=TabAdvanced then
     LTotalMemory.Caption:=IntToStr(TotalMemoryAllocated);
end;
...
begin
  ...
  if OpenGL2.DescribePixelFormat(tmp,PixelFormat,SizeOf(PFDescriptor),PFDescriptor) then
     if OpenGL2.SetPixelFormat(tmp,PixelFormat,{$IFNDEF CLR}@{$ENDIF}PFDescriptor) then
     begin
