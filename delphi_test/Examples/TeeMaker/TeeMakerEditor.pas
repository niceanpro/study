{******************************************}
{ TeeMaker 2.0                             }
{ Copyright (c) 2002-2011 by David Berneda }
{ All Rights Reserved                      }
{******************************************}
unit TeeMakerEditor;
{$I TeeDefs.inc}
...
type
  ...
  private
    ...
    (*
    {$IFNDEF CLR}
    procedure FilterInspector(Sender:TObjectInspector; Instance:TObject;
...
begin
  {$IFDEF CLR}inherited {$ENDIF}Create(AOwner);
  AddMaker(AMaker);
end;
...
begin
  ...
  begin
    ...
    {$IFNDEF CLR}TBlocksAccess{$ENDIF}(Maker.Blocks).OnItemsChanged:=BlocksItemsChanged;
    {$IFNDEF CLR}TBlocksAccess{$ENDIF}(Maker.Blocks).OnLoaded:=BlocksLoaded;
...
begin
  ...
  AddFormTo(IBlockEditor,PanelEditor);
...
begin
  ...
  begin
    ...
    {$IFNDEF CLR}TBlocksAccess{$ENDIF}(Maker.Blocks).OnItemsChanged:=nil;
    {$IFNDEF CLR}TBlocksAccess{$ENDIF}(Maker.Blocks).OnLoaded:=nil;
...
var t : Integer;
begin
  ParentNode:=TreeBlocks.Items.AddChildObject(ParentNode,{$IFNDEF CLR}TBlockAccess{$ENDIF}(ABlock).TitleOrName,ABlock);
  {$IFNDEF CLR}TBlockAccess{$ENDIF}(ABlock).IData:=ParentNode;
...
begin
  if Assigned(ABlock) then
     result:=TTreeNode({$IFNDEF CLR}TBlockAccess{$ENDIF}(ABlock).IData)
  else
     result:=nil;
end;
...
begin
  ...
  begin
    {$IFNDEF CLR}TBlockAccess{$ENDIF}(result).PrepareForGallery;
    DoAddClick(result,TBlockClasses.BlockDescription(result.ClassName));
    {$IFNDEF CLR}TBlocksAccess{$ENDIF}(CurrentMaker.Blocks).SetBlockName(result);
...
begin
  {$IFNDEF CLR}TBlocksAccess{$ENDIF}(CurrentMaker.Blocks).SetBlockName(ABlock);
...
procedure TMakerEditor.InitFirstMaker;
  ...
  begin
    ...
    with {$IFNDEF CLR}TCommAccess{$ENDIF}(TeeCommander1) do
...
function TMakerEditor.SaveBlocksToFile(const AMaker:TMaker; const FileName:String):Boolean;
  ...
  begin
    ...
    if result then
    begin
      tmp:=GetFileAttributes({$IFNDEF CLR}PChar{$ENDIF}(FileName));
      tmp:=tmp and (not faReadOnly);
      SetFileAttributes({$IFNDEF CLR}PChar{$ENDIF}(FileName),tmp);
    end;
  end;
...
begin
  ...
  begin
    ...
    {$IFNDEF CLR}TBlocksAccess{$ENDIF}(CurrentMaker.Blocks).SetBlockName(tmpNew);
    ...
    begin
      ...
      {$IFDEF CLR}
      tmpP:=Location.Point;
      tmpP.X:=tmpP.X+Size.X+(Size.X*0.5);
      Location.Point:=tmpP;
      {$ELSE}
...
begin
  ...
  begin
    ...
    if TeeSelectFolder(TeeMsg_SelectLibraryFolder,'',tmp) then
       LibraryPath:=tmp;
  end;
end;
...
{  if Value then
  ...
  begin
    ...
    {$IFNDEF CLR}TMakerAccess{$ENDIF}(CurrentMaker).DeleteAllLists;
...
(*
{$IFNDEF CLR}
procedure TMakerEditor.FilterInspector(Sender:TObjectInspector; Instance:TObject;
                             const PropertyName:String; var Accept:Boolean);
begin
  if (PropertyName='Tag') or (PropertyName='Name') then
       Accept:=False
  else
  if Instance is TPropertyAnimation then
     if (PropertyName='Instance') then
        Accept:=False;
end;
{$ENDIF}
*)
...
{
procedure TMakerEditor.InspectorChanged(Sender:TTeeInspector; Item:TInspectorItem);
begin
  MarkDirty;
end;
}
...
begin
  ...
  try
    ...
    begin
      ...
      {$IFNDEF CLR}TAnimationAccess{$ENDIF}(Animation).FixupReferences(Self.CurrentMaker.Blocks.CurrentSource);

      if Animation is TPropertyAnimation then
      with {$IFNDEF CLR}TPropertyAnimationAccess{$ENDIF}(Animation) do
...
begin
  if EditTeeGradient(Self,CurrentMaker.Gradient) then
     MarkDirty;
end;

procedure TMakerEditor.Backimage1Click(Sender: TObject);
begin
  if EditTeePicture(Self,CurrentMaker.BackImage) then
     MarkDirty;
end;
...
begin
  ...
  {$IFNDEF CLR} // TODO
  if not CreateProcess(nil, {$IFNDEF CLR}PChar{$ENDIF}(tmp),
...
begin
  ...
  begin
    ...
    begin
      ...
      AddFormTo(IAnimEditor, PanelBig);
    end;
  end;
...
begin
  ...
  begin
    StatusBar1.SimpleText:={$IFNDEF CLR}TBlockAccess{$ENDIF}(TCustomBlock(Sender)).TitleOrName;
...
procedure TMakerEditor.ScrollBar1Change(Sender: TObject); {$IFDEF CLR}unsafe;{$ENDIF}
var tmp : GLMat;
begin
...
procedure TMakerEditor.ScrollBar2Change(Sender: TObject); {$IFDEF CLR}unsafe;{$ENDIF}
var tmp : GLMat;
begin
...
procedure TMakerEditor.PageExtrasChange(Sender: TObject);
  ...
  begin
    ...
    tmpNum:={$IFNDEF CLR}TGLCanvasAccess{$ENDIF}(tmp).CachedFonts;
...
begin
  ...
  begin
    ...
    begin
      ...
      AddFormTo(IFloorEditor,TabFloor);
    end;
...
begin
  ...
  begin
    ...
    AddFormTo(IColorPalette,TabColors);
...
procedure TMakerEditor.WMDROPFILES(var Message: TWMDROPFILES);
  ...
  begin
    ...
    begin
      ...
      result:=FileName{$IFDEF CLR}.ToString{$ENDIF};
    end
    else
      result:='';
...
begin
  if TeeEditStrings(Self,CurrentMaker.Blocks.Events) then
     MarkDirty;
end;
...
begin
  ...
  {$IFDEF CLR}
  tmpP:=tmp.Location.Point;
  tmpP.X:=tmpP.X+tmp.Size.X+(tmp.Size.X*0.5);
  tmp.Location.Point:=tmpP;
...
begin
  CopyFile({$IFNDEF CLR}PChar{$ENDIF}(Source),
           {$IFNDEF CLR}PChar{$ENDIF}(Dest),False);
end;
  ...
  begin
    ...
    begin
      if Blocks[t].Format.Texture.PictureLink<>'' then
         {$IFNDEF CLR}TBlockTextureAccess{$ENDIF}(Blocks[t].Format.Texture).SetEmbeddedPicture;
...
begin
  ...
  begin
    ...
    if TeeSelectFolder(TeeMsg_SelectFolder,'',tmpFolder) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      AddFormTo(ICameraEditor,PanelCamera);
    end;
...
begin
  ...
  begin
    ...
    AddFormTo(IKinematics,PanelBig);
  end;
