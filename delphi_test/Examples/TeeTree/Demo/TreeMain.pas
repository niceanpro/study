...
uses
   ...
   AnimatedGIF,
   {$ENDIF}
   Base, TeeAbout, TeeProcs, TeeTree, TreeFlow, TeeGDIPlus;
...
var p   : Integer;
  ...
  begin
    While p<=Len do
    begin
      if {$IFDEF D12}CharInSet(tmp[p],Valid)
         {$ELSE}
         ({$IFDEF CLR}AnsiChar{$ENDIF}(tmp[p]) in Valid)
         {$ENDIF}
           then
         break
      else
    ...
    p2:=p;
    While (p<=Len) and
          {$IFDEF D12}CharInSet(tmp[p],Valid)
          {$ELSE}
          ({$IFDEF CLR}AnsiChar{$ENDIF}(tmp[p]) in Valid)
          {$ENDIF} do
            Inc(p);
