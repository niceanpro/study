...
uses
  ...
  {$IFDEF UCL}
  UGraphics, UControls, UForms, UDialogs, UImgList, UMenus, UComCtrls,
  UStdCtrls, UExtCtrls, UButtons, UGrids,
  {$ELSE}
  {$IFDEF CLX}
...
{$IFDEF CLX}
var tmpThemes : TMenuItem;

  procedure AddMenuStyle(const AName:String; IsDefault:Boolean);
  var tmp : TMenuItem;
  begin
...
begin
  ...
  {$IFDEF UCL}
  for t:=Low(cStyles) to High(cStyles) do
      AddMenuStyle(cStyles[t],cStyles[t]=Application.Style.DefaultStyle);
  {$ELSE}
  for AStyle:=Low(TDefaultStyle) to High(TDefaultStyle) do
      AddMenuStyle(GetEnumName(TypeInfo(TDefaultStyle),Ord(AStyle)),
                   AStyle=Application.Style.DefaultStyle);
  {$ENDIF}
...
begin
  ...
  {$IFDEF UCL}
  Application.Style.DefaultStyle:=StripHotkey(Item.Caption);
  {$ELSE}
