...
type
  ...
  public
    Constructor Create; {$IFDEF CLR}override;{$ENDIF}
      ...
      Case Strings[t][1] of
        ...
        'F': begin { embedded function ie: sin(x) }
               ...
               case tmpFunc of
                 ...
                 6: result:=Abs(result);
                 7: result:={$IFDEF CLR}Math.{$ENDIF}Tan(result*Pi/180.0);
               else
                 Error;
               end;
             end;
        'O': begin
               ...
               case tmp of
                 ...
                 '-': result:=result-Value;
                 '/': result:=result/Value;
                 '^': result:={$IFDEF CLR}Math.Pow{$ELSE}IntPower{$ENDIF}(result,Round(Value));
               else Error;
               end;
             end;
      else
...
begin
  ...
  t:=1;
  Repeat
    if tmpSt[t]=' ' then {$IFDEF CLR}tmpSt:=tmpSt.Remove(t,1){$ELSE}System.Delete(tmpSt,t,1){$ENDIF}
                    else Inc(t);
...
initialization
  ...
  TeeSources.Add({$IFDEF CLR}TObject{$ENDIF}(TTeeExpressionSource));
finalization
  TeeSources.Remove({$IFDEF CLR}TObject{$ENDIF}(TTeeExpressionSource));
end.
