...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure GridMode1Click(Sender: TObject);
  private
...
begin { initialization when form is shown }
  ...
  {$IFNDEF TEENOTHEMES}
  TeeNewChartTheme:=TeeReadIntegerOption(TeeMsg_DefaultOfficeTheme,8);
...
begin { load a Chart from file, and setup several things }
  ...
  if not FileExists(AFileName) then
     Raise EFOpenError.CreateFmt({$IFDEF D6}SFOpenErrorEx{$ELSE}'Cannot open file "%s". %s'{$ENDIF},
...
procedure TMainForm.GridMode1Click(Sender: TObject);
var tmp : Boolean;
begin
  GridMode1.Checked:=not GridMode1.Checked;

  tmp:=not GridMode1.Checked;

  ControlBar1.Visible:=tmp;
  ChartGalleryPanel1.Visible:=tmp;
  SplitterGallery.Visible:=tmp;
  Panel1.Visible:=tmp;
  Splitter1.Visible:=tmp;
  PageControl1.Visible:=tmp;

  if tmp then
     IGrid.Free
  else
  begin
    IGrid:=TImport_Form.Create(nil);
    IGrid.Panel3.Hide;
    IGrid.CBFormat.ItemIndex:=4;

    IGrid.Chart1.Assign(Self.DBChart1);
