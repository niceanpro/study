...
uses
   ...
   {$IFDEF CLR}
   System.ComponentModel, System.Text, System.IO, System.Runtime.InteropServices,
   {$ELSE}
   jpeg,
   {$ENDIF}
...
procedure SendEmail(Text,Subject:AnsiString); {$IFDEF CLR}unsafe;{$ENDIF}
{$IFNDEF LINUX}
...
begin
  ...
  try
    ...
    begin
      ...
      //lpszAddress:={$IFDEF CLR}IntPtr(@{$ELSE}PChar({$ENDIF}s);

      {$IFDEF CLR}
      lpszAddress:=Marshal.StringToBSTR(s);
      {$ELSE}
      lpszAddress:={$IFNDEF CLR}{$IFDEF D12}PAnsiChar{$ELSE}PChar{$ENDIF}{$ENDIF}(s);
      {$ENDIF}
